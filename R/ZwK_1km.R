...
d <- read.csv("zulawy-wkolo_1km.csv", sep = ';', row.names=NULL,  header=T, na.string="NA");
...
ggplot(d, aes(x = tdistkm)) +
  ...
  ggtitle("Żuławy w koło 2017 (1 km averages)") +
  theme(legend.position=c(.6, .9)) +
  theme(legend.text=element_text(size=12));

ggsave(file="ZwK_1km_slope.pdf", width=20, height=5)
